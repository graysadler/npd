 ...
 */
function mdbigstream_install_tasks(&$install_state) {

  $tasks = array();

  // Add our custom CSS file for the installation process
  drupal_add_css(drupal_get_path('profile', 'mdbigstream') . '/mdtheme.css');
 ...
 */
function mdbigstream_form_install_configure_form_alter(&$form, $form_state) {
  $form['site_information']['site_name']['#default_value'] = 'MD Bigstream';
  $form['#submit'][] = 'mdbigstream_install_configure_form_submit';
}

/**
 * Forms API submit for the site configuration form.
 */
function mdbigstream_install_configure_form_submit($form, &$form_state) {
  global $user;
  $sql_file = dirname(__FILE__) . '/mdbigstream.sql';
  $count = mdbigstream_import_sql($sql_file);
...
function mdbigstream_import_sql($filename) {
  global $databases;
  $databasesName = array(
    "pgsql" => "PostgreSQL",
    "sqlite" => "SQLite"
  ...
  if ($file) {
    ...
    if ($connect) {
      while (!feof($file)) {
        ...
        if (preg_match('|;$|', $line)) {
          try {
            $connect->exec(mdbigstream_prefixTables($sql_command));
...
function mdbigstream_prefixTables($sql) {
  global $databases;
